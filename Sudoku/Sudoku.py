...
class SudokuUI(Frame):
    ...
    def __cell_clicked(self, event): # always takes event parameter, always called when user clicks mouse
        ...
        if (MARGIN < x < WIDTH - MARGIN and MARGIN < y < HEIGHT - MARGIN): # on the game board
            ...
            elif self.game.puzzle[row][col] ==0 or self.game.puzzle[row][col] != self.game.start_puzzle[row][col]:   # can only click on "blank spaces"
    ...
    def __draw_cursor(self):
        ...
        if self.row >= 0 and self.col >= 0:         # put in focus
            ...
            self.canvas.create_rectangle(
                x0, y0, x1, y1,
                outline="red", tags="cursor")       # draw rectangle around focused cell
    ...
    def __key_pressed(self, event):
        ...
        if self.row >= 0 and self.col >= 0 and event.char in "1234567890":
            ...
            self.__draw_cursor()    # deletes red outline of cursor
